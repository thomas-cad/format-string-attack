...
La différence ne réside que dans le dernier octet. Il est donc inutile de connaître l'adresse de base aléatoire. Il suffit d'écraser le dernier octet \code{0xf3} par \code{0xf9} pour réussir le détournement de manière fiable à 100\%.

\subsection{Charge utile finale (Trigger)}

Le payload final injecté par le script est construit ainsi :

\[ \text{Payload} = \text{"\%249c\%7\\\$hhn"} \]

\begin{itemize}
    \item \textbf{\%249c} : Le script calcule \code{addr\_ret \& 0xff}, ce qui donne \code{0xf9} (soit 249 en décimal). La fonction \code{printf} affiche donc 249 caractères de bourrage pour ajuster son compteur interne.
    ...
    \item \textbf{hhn} : Le spécificateur \code{n} écrit le nombre de caractères affichés (249) à l'adresse pointée. Le modificateur \code{hh} assure que l'écriture se fait sur \textbf{un seul octet}.
\end{itemize}
